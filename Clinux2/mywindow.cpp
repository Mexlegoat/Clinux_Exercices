...
void MyWindow::on_pushButtonLogin_clicked()
{
  ...
  if (nouvelUtilisateur == 1)
  {
    ...
    {
      setResultat("Erreur lors de la création de l'utilisateur");
  ...
  else
  {
    if (pos != -1 || pos != 0)
    {
        ...
        else if (verifieMotDePasse(pos, motDePasse) == 0)
        {
...
void MyWindow::on_pushButtonAfficheFichier_clicked()
{
  ...
  UTILISATEUR user[50];
  int cpt = listeUtilisateurs(user);
  for (int i = 0; i < cpt; i++)
  {
    ajouteTupleTableUtilisateurs(user[i].nom, user[i].hash);
  }
